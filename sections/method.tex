...
\begin{figure*}[t!]
  ...
  \caption{\textbf{Illustration of the proposed \textbf{\model{}} framework.} 
    ...
    % Upper right part depicts the progress of LLMs enhanced region-aware instruction.
  }
  \label{fig:arch}
\end{figure*}
...
% After an extensive examination of recent generative models, we have determined that the diffusion model~\cite{rombach2022high} aligns with our requirements. 
...
% By incorporating this condition, the classifier-free guidance~\cite{ramesh2022hierarchical} adjusts the original predicted noise $\bm{\epsilon}_\theta(x_t,\varnothing)$ towards the guidance signal $\bm{\epsilon}_\theta(x_t,c)$, formulated as 
% \begin{equation}
%     \hat{\bm{\epsilon}}_{\theta}(x_t, c) = 
%     \bm{\epsilon}_\theta(x_t,\varnothing) + 
%     w(\bm{\epsilon}_\theta(x_t,c) - \bm{\epsilon}_\theta(x_t,\varnothing)) .
% \end{equation}
...
% \subsubsection{Adaptor Block for Spatial Feature Modulation.}
% \subsubsection{Progressive Alignment and Instruction via Asynchronous Diffusion.} 
% \subsection{Multi-Modal Instruction}
